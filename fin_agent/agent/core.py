...
class FinAgent:
    ...
    def _get_system_content(self):
        ...
        return (
            ...
            "**Rule 6 — 基本面分析 & 事件**\\n"
            "财务指标分析 (ROE/ROA/毛利率/资产负债率等) → 'get_financial_indicators(ts_code)'.\\n"
            "同行业估值对比 (PE/PB横向比较) → 'get_peer_comparison(ts_code)'.\\n"
            "分红历史 (历年分红方案、股息率) → 'get_stock_dividend_history(ts_code)'.\\n"
            "公告与事件日历 (财报披露、重大事项、解禁等) → 'get_event_calendar(symbol_type, date)'.\\n"
            "当用户询问'财务健康'、'估值高不高'、'和同行比'时，同时调用 get_financial_indicators 和 get_peer_comparison。\\n"
            "当用户是保守型投资者询问推荐时，主动调用 get_stock_dividend_history 获取分红数据。\\n\\n"
